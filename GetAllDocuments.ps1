...
param(		
	...
	[string] $Fields = "Data_x0020_documento,Anno,Tipologia_x0020_documento,Argomento,Modified,Sottotitolo,Abstract,Unit_x00e0__x0020_organizzativa_x0020_Autore"
)
#endregion
...
try
{
	...
	$headers = "Area,Subsite,DocumentLibrary,DocumentID,Title,FileName,Link"
	...
	foreach($web in $webs)
	{
		...
		$docLibraries = $web.Lists | Where-Object BaseType -eq ([Microsoft.SharePoint.SPBaseType]::DocumentLibrary)
		$docsProgressTracker = [Progress]::new($docLibraries.Count, $true)		
		foreach($docLibrary in $docLibraries)
		{				
			$docsProgressTracker.WriteProgress("Analyzing document library [$($doclibrary.Title)]")
			$Log.WriteLog("Analyzing document library $($docLibrary.Title)")
			$documents = $docLibrary.Items    
			foreach($document in $documents)
			{
				$csvLine = "`"$($document.Web.Title)`",$($document.Web.ServerRelativeUrl),`"$($docLibrary.Title)`",$($document.ID),`"$($document.Title)`",`"$($document.File.Name)`",`"$("$($site.Url)/$($document.File.Url)")`""
				foreach($customField in $customFields) {
					$fieldValue = $document["$customField"]
